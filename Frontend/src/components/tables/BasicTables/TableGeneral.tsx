import { useMemo } from "react";
import Label from "../../form/Label";
import Input from "../../form/input/InputField";
import {
    Table,
    TableBody,
    TableCell,
    TableHeader,
    TableRow,
} from "../../ui/table";
import { Hashtag, DollarLineIcon, CalenderIcon } from "../../../icons";

export default function TableGeneral({
    leads,
    customers,
}: {
    leads: any[];
    customers: any[];
}) {
    const rows = useMemo(() => {
        return leads.map((lead) => {
            const customer = customers.find(
                (c) => Number(c.id) === Number(lead.customerId)
            );

            const activity = lead.activities?.find(
                (a: any) => Number(a.activityTypeId) === 1
            );

            const formattedDate = activity
                ? new Date(activity.createdAt).toLocaleDateString("es-CO")
                : "—";

            return {
                id: lead.id,
                customer: customer?.name ?? "Cliente desconocido",
                customerCode: customer?.id,
                priceCode: lead.cotization,
                priceValue: Number(lead.quotedValue ?? 0),
                billValue: Number(lead.billingValue ?? 0),
                date: formattedDate,
            };
        });
    }, [leads, customers]);

    const totalCotizado = useMemo(() => {
        return rows.reduce((acc, row) => {
            const value = Number(row.priceValue) || 0;
            return acc + value;
        }, 0);
    }, [rows]);


    const totalFacturado = useMemo(() => {
        return rows.reduce((acc, row) => {
            const value = Number(row.billValue) || 0;
            return acc + value;
        }, 0);
    }, [rows]);

    return (
        <>
            <div className="flex gap-3 items-center w-fit">
                <Label>Total Cotizado: </Label>
                <Input
                    type="text"
                    placeholder="Total Cotizado"
                    value={totalCotizado.toLocaleString("es-CO")}
                    onChange={(value) => value}
                />
            </div>
            <div className="flex gap-3 items-center w-fit">
                <Label>Total Facturado: </Label>
                <Input
                    type="text"
                    placeholder="Total Facturado"
                    value={totalFacturado.toLocaleString("es-CO")}
                    onChange={(value) => value}
                />
            </div>
            <div className="overflow-hidden rounded-xl border border-gray-400 bg-white dark:border-white/[0.05] dark:bg-white/[0.05]">
                <div className="w-full overflow-x-auto md:overflow-visible">
                    <Table>
                        {/* Table Header */}
                        <TableHeader className="border-b border-gray-600 dark:border-white/[0.80] bg-white dark:bg-gray-950">
                            <TableRow>
                                <TableCell
                                    isHeader
                                    className="px-3 py-3 font-semibold text-gray-700 dark:text-white/90"
                                >
                                    Caso
                                </TableCell>

                                <TableCell
                                    isHeader
                                    className="hidden lg:table-cell px-3 py-3 font-semibold text-gray-700 dark:text-white/90"
                                >
                                    Codigo Del Cliente
                                </TableCell>

                                <TableCell
                                    isHeader
                                    className="px-3 py-3 font-semibold text-gray-700 dark:text-white/90"
                                >
                                    Cliente
                                </TableCell>

                                <TableCell
                                    isHeader
                                    className="px-3 py-3 font-semibold text-gray-700 dark:text-white/90"
                                >
                                    Código Cotización
                                </TableCell>

                                <TableCell
                                    isHeader
                                    className="px-3 py-3 font-semibold text-gray-700 dark:text-white/90"
                                >
                                    Total Facturado
                                </TableCell>

                                <TableCell
                                    isHeader
                                    className="hidden lg:table-cell px-3 py-3 font-semibold text-gray-700 dark:text-white/90"
                                >
                                    Fecha De Creación
                                </TableCell>
                            </TableRow>
                        </TableHeader>

                        {/* Table Body */}
                        <TableBody className="divide-y divide-gray-100 dark:divide-white/[0.2] text-gray-600 dark:text-white/90">
                            {rows.map((order) => (
                                <TableRow
                                    key={order.id}
                                    className="hover:bg-gray-50 dark:hover:bg-white/[0.05]"
                                >
                                    {/* Caso */}
                                    <TableCell className="px-3 py-3 text-sm">
                                        <span className="flex gap-1 items-center justify-center">
                                            <Hashtag />
                                            {order.id}
                                        </span>
                                    </TableCell>

                                    {/* Cliente */}
                                    <TableCell className="hidden lg:table-cell px-3 py-3 text-sm">
                                        <span className="flex gap-1 items-center justify-center">
                                            <Hashtag />
                                            {order.customerCode}
                                        </span>
                                    </TableCell>

                                    <TableCell className="px-3 py-3 text-sm font-medium text-center">
                                        {order.customer}
                                    </TableCell>

                                    {/* Código Cotización */}
                                    <TableCell className="px-3 py-3 text-sm">
                                        <span className="flex gap-1 items-center justify-center">
                                            <Hashtag />
                                            {order.priceCode}
                                        </span>
                                    </TableCell>

                                    {/* Total */}
                                    <TableCell className="px-3 py-3 text-sm">
                                        <span className="flex gap-1 items-center justify-center">
                                            <DollarLineIcon />
                                            {order.priceValue.toLocaleString("es-CO")}
                                        </span>
                                    </TableCell>

                                    {/* Fecha */}
                                    <TableCell className="hidden lg:table-cell px-3 py-3 text-sm">
                                        <span className="flex gap-1 items-center justify-center">
                                            <CalenderIcon />
                                            {order.date}
                                        </span>
                                    </TableCell>
                                </TableRow>
                            ))}
                        </TableBody>
                    </Table>
                </div>
            </div>
        </>
    );
}
