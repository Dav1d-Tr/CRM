...
const Alert: React.FC<AlertProps> = ({
  ...
  const variantClasses = {
    ...
    error: {
      container:
        "border-error-500 bg-error-100 dark:border-error-500/30 dark:bg-error-500/90 lg:dark:bg-error-500/50",
      icon: "text-gray-900 dark:text-gray-200",
    },
    warning: {
      container:
  ...
  return (
    <div className="fixed top-20 inset-x-0 z-50 flex justify-center">
      <div
        className={`rounded-xl border p-4 max-w-sm w-full ${variantClasses[variant].container}`}
      >
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            {showLink && (
              <Link
                to={linkHref}
                className="inline-block mt-3 text-sm font-medium text-gray-300 underline dark:text-gray-200"
              >
                {linkText}
              </Link>
            )}
