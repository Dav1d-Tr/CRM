// src/utils/exportGeneralExcel.ts
...
export async function exportGeneralExcel(
    leads: Lead[],
    customers: Customer[],
    contacts: Contact[],
    countries: Country[],
    ...
    // Precargar estados 1 al 8 aunque estén en cero
    for (let estadoId = 1; estadoId <= 8; estadoId++) {
    ...
    for (const lead of leads) {
        ...
        const valorFacturado = num(lead.billingValue);
        ...
        r.cantidad += 1;
        r.total += valor;
        r.totalFacturado += valorFacturado;
    }
    ...
    const totalFacturado = leads.reduce((acc, l) => acc + num(l.billingValue), 0);
    ...
    resumen.addRow({ indicador: "Total Facturado", valor: totalFacturado });
    ...
    resumen.getColumn(3).width = 50;
    resumen.getColumn(4).width = 50;

    startRow += 1;

    // Headers
    resumen.getRow(startRow).values = ["Estado", "Cantidad De Leads", "Total Cotizado", "Total Facturado"];
    ...
    for (const r of resumenPorEstado) {
        resumen.addRow([r.estado, r.cantidad, r.total, r.totalFacturado]);
        totalLeadsGeneral += r.cantidad;
        totalCotizadoGeneral += r.total;
        totalFacturadoGeneral += r.totalFacturado;
    }
    const totalRow = resumen.addRow(["TOTAL GENERAL", totalLeadsGeneral, totalCotizadoGeneral, totalFacturadoGeneral]);
    totalRow.font = { bold: true };

    resumen.getColumn(2).numFmt = "0";
    resumen.getColumn(3).numFmt = "#,##0.00";
    resumen.getColumn(4).numFmt = "#,##0.00";
    ...
    const stayLeads = leads.filter(l => Number(l.stateId) != 8)
    ...
    for (const lead of stayLeads) {
        const customerId = String(lead.customerId);
    ...
    // ================= CLIENTES PERDIDOS =================
    const clientesperdidosSheet = workbook.addWorksheet("CLIENTES PERDIDOS");

    clientesperdidosSheet.columns = [
        { header: "Código del cliente", key: "id", width: 20 },
        { header: "Cliente", key: "name", width: 30 },
        { header: "País", key: "pais", width: 20 },
        { header: "Ciudad", key: "ciudad", width: 20 },
        { header: "Motivo de cancelación", key: "cancel", width: 60 },
    ];

    styleHeaderAt(clientesperdidosSheet, 1);
    enableFilterOnHeader(clientesperdidosSheet, 1);
    clientesperdidosSheet.views = [{ state: "frozen", ySplit: 1 }];
    clientesperdidosSheet.getColumn("cancel").alignment = { wrapText: true, vertical: "top" };

    const lostLeads = leads.filter(l => Number(l.stateId) === 8)
    ...
    for (const lead of lostLeads) {
        ...
        if (!customer) continue;
        clientesperdidosSheet.addRow({
            id: Number(customer.id),
            name: customer.name,
            pais: countryMap.get(String(customer.countryId)) ?? "",
            ciudad: cityMap.get(String(customer.cityId)) ?? "",
            cancel: lead.cancellationReason ?? "——",
        });
        processedCustomers.add(customerId);
    }

    styleDataBody(clientesperdidosSheet, 2, [5]);
    styleHeaderAt(clientesperdidosSheet, 1);

    // ================= INFORME GLOBAL GENERAL =================
    const sheet = workbook.addWorksheet("INFORME GLOBAL GENERAL");
    ...
    sheet.columns = [
        ...
        { header: "Motivo de Cancelación", key: "can", width: 100 }
    ];
    ...
    sheet.getColumn("can").alignment = { wrapText: true, vertical: "top" };
    ...
    for (const lead of leads) {
        ...
        const fechaFacturacion = getActivityDate(lead, 11);
    ...
    sheet.getColumn("fechaCreacion").numFmt = "dd/mm/yyyy";
    sheet.getColumn("fechaCotizacion").numFmt = "dd/mm/yyyy";
    sheet.getColumn("fechaFacturacion").numFmt = "dd/mm/yyyy";
    sheet.getColumn("valorCot").numFmt = "#,##0.00";

    styleDataBody(sheet, 2, [22, 23, 24]);
    styleHeaderAt(sheet, 1);

    // ================= PROSPECCIÓN =================
    const prospeccionsheet = workbook.addWorksheet("PROSPECCIÓN");

    prospeccionsheet.columns = [
        { header: "Caso", key: "caso", width: 10 },
        { header: "Código del cliente", key: "codCliente", width: 22 },
        { header: "Cliente", key: "cliente", width: 28 },
        { header: "País", key: "pais", width: 18 },
    ...
    styleHeaderAt(prospeccionsheet, 1);
    enableFilterOnHeader(prospeccionsheet, 1);
    prospeccionsheet.views = [{ state: "frozen", ySplit: 1 }];
    prospeccionsheet.getColumn("contactos").alignment = { wrapText: true, vertical: "top" };
    prospeccionsheet.getColumn("obs").alignment = { wrapText: true, vertical: "top" };

    const leadsEstado1 = leads.filter(l => Number(l.stateId) === 1);

    for (const lead of leadsEstado1) {
        const customer = usedCustomers.find(
            (c) => String(c.id) === String(lead.customerId)
        );
        ...
        const row = prospeccionsheet.addRow({
            caso: Number(lead.id),
            codCliente: Number(lead.customerId),
            cliente: customer?.name ?? "",
            pais: countryMap.get(String(customer?.countryId)) ?? "",
    ...
    prospeccionsheet.getColumn("valorCot").numFmt = "#,##0.00";

    styleDataBody(prospeccionsheet, 2, [22, 23]);
    styleHeaderAt(prospeccionsheet, 1);
    ...
    styleDataBody(cotizacionsheet, 2, [22, 23]);
    styleHeaderAt(cotizacionsheet, 1);
    ...
    styleDataBody(ingenieriasheet, 2, [22, 23]);
    styleHeaderAt(ingenieriasheet, 1);
    ...
    styleDataBody(seguimientosheet, 2, [22, 23]);
    styleHeaderAt(seguimientosheet, 1);
    ...
    styleDataBody(Kickoffsheet, 2, [22, 23]);
    styleHeaderAt(Kickoffsheet, 1);
    ...
    styleDataBody(facturadosheet, 2, [22, 23]);
    styleHeaderAt(facturadosheet, 1);

    // ================= CANCELADOS =================
    const canceladosheet = workbook.addWorksheet("CANCELADOS");
    canceladosheet.columns = [
        { header: "Caso", key: "caso", width: 10 },
        { header: "Código del cliente", key: "codCliente", width: 22 },
        { header: "Cliente", key: "cliente", width: 28 },
        { header: "País", key: "pais", width: 18 },
    ...
    styleHeaderAt(canceladosheet, 1);
    enableFilterOnHeader(canceladosheet, 1);
    canceladosheet.views = [{ state: "frozen", ySplit: 1 }];
    canceladosheet.getColumn("contactos").alignment = { wrapText: true, vertical: "top" };
    canceladosheet.getColumn("obs").alignment = { wrapText: true, vertical: "top" };
    canceladosheet.getColumn("can").alignment = { wrapText: true, vertical: "top" };

    const leadsEstado8 = leads.filter(l => Number(l.stateId) === 8);

    for (const lead of leadsEstado8) {
        const customer = usedCustomers.find(
            (c) => String(c.id) === String(lead.customerId)
        );
        const contactosCliente = usedContacts
        ...
        const row = canceladosheet.addRow({
            caso: Number(lead.id),
            codCliente: Number(lead.customerId),
            cliente: customer?.name ?? "",
            pais: countryMap.get(String(customer?.countryId)) ?? "",
    ...
    canceladosheet.getColumn("valorCot").numFmt = "#,##0.00";

    styleDataBody(canceladosheet, 2, [22, 23, 24]);
    styleHeaderAt(canceladosheet, 1);
